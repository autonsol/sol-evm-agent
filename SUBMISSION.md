...
### 2. Tiered Exit Parameters by Risk Band (Phase 5 — v1.51.0 calibration)
...
- **Phase 22** (v1.58.0, 2026-04-04 4:49 AM): Positive drift hold extension **+2h when pnlPct > 1% at expiry** — Phase 21 exit breakdown: 4/7 trades were time_expired at +2.3% avg. These tokens were trending positively but too slowly for the 6h window. Root cause: Phase 0 trailing stop triggers at +8% gain; positive drift tokens (ending at +1-3%) never reach that trigger. Fix: when a position is profitable at the 6h deadline, extend once by +2h (extendedHold flag prevents infinite extension; SL unchanged). Evidence at 8 trades: **37.5% WR, E=+0.37%, profit_factor 1.36** — exit breakdown: trailing_stop 3x avg **+3.7%** (extended holds converting to real exits ✓), time_expired 5x avg -1.7% (tokens below +1% threshold, correctly gated). Mechanism confirmed. See `/stats → extended_hold_breakdown` for which positions used the extension.
- **Phase 23** (v1.59.0, 2026-04-05 2:49 AM): **24h trend alignment filter** — Phase 22 residual: 5/8 time_expired exits at -1.7% avg. Hypothesis: these are *dead-cat-bounce* entries — tokens with +2% 1h momentum but in a severe 24h downtrend (-25%+). The 1h filter detects direction but not context: a token that's -35% over 24h and +2.5% over 1h is bouncing off a low, not establishing a new uptrend. Fix: block entries where `price_change_24h < -25%` (tokens in sustained distribution). Threshold rationale: -25%+ = institutional distribution event; -10% to -25% = elevated volatility but recoverable (allow through). Expected: 2-4 fewer time_expired losers/week; overall E improves toward Phase 18 benchmark (+1.69%). Filter uses existing DexScreener 24h data already fetched per scan.
...
**Updated 2026-04-05:** 16 strategy epochs total. Key milestones: Phase 17 (momentum threshold 3.0x→2.0x) broke the 7h entry deadlock. **Phase 18 (best epoch):** 9 trades, **55.6% WR, +1.7% avg PnL, E=+1.69%/trade** — first consistently positive-expectancy epoch. **Phase 22:** 8 trades, 37.5% WR, E=+0.37% — mechanism confirmed: trailing_stop 3x avg +3.7% (extended holds reaching real exits ✓). **Phase 23 (CURRENT, v1.59.0, deployed 2026-04-05T06:49Z):** 24h trend alignment filter — residual Phase 22 time_expired losses (-1.7% avg) diagnosed as dead-cat-bounce entries (token -25%+ in 24h, 1h bounce ≠ new uptrend). Fix: `price_change_24h >= -25%` entry gate. Expected: 2-4 fewer time_expired losers/week, approach Phase 18 benchmark (+1.69% E). Extended hold breakdown now tracked in `/stats → phase_22 → extended_hold_breakdown`. See `/stats → strategy_epochs`.*
...
- [x] 15 strategy epochs tracked, 144 trades accumulated ✅
- [x] Phase 22 positive drift extension live (v1.58.0, 8 trades, E=+0.37%, mechanism confirmed ✅)
- [x] Phase 23 24h trend alignment filter (v1.59.0, deployed 2026-04-05T06:49Z) ✅
...
*Agent loop: v1.59.0 | Signal adapter: v1.2.0 | ERC-8004: EIP draft v0.3*
...
*Last stats update: 2026-04-05 02:49 EST — **145 all-time trades, 47.6% WR, -0.2% avg** | Phase 1: +69.9% (57.1% WR, E=+3.33%) | Phase 3: 56.7% WR | Phase 5: 56 trades, 48.2% WR | **Phase 18 (BENCHMARK):** 9 trades, **55.6% WR, +1.7% avg, E=+1.69%**. | **Phase 22 (v1.58.0):** 8 trades, 37.5% WR, E=+0.37% — trailing_stop 3x avg +3.7% (extended hold mechanism confirmed ✓). | **Phase 23 LIVE (v1.59.0, deployed 2026-04-05T06:49Z):** 24h trend alignment filter — blocks dead-cat-bounce entries (price_change_24h < -25%). Diagnoses residual time_expired -1.7% losses from Phase 22 as sustained-downtrend bounces. 16 strategy epochs tracked in /stats. 7 days to April 12 deadline. | Pitch deck: PITCH-DECK.md*
