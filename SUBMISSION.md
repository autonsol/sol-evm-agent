...
- **Phase 21** (in progress, diagnosis 2026-04-03): Phase 18 data shows "sideways drift" time_expired exits (PnL –1% to +3%) are a distinct failure mode from "near-TP stall" time_expired exits (PnL +5%+). TIBBIR drifted 6h to –0.2% → 20min cooldown → still re-entered → SL –8.5%. Hypothesis: time_expired exits within ±3% of entry deserve a longer 60min blacklist (the token burned the full hold window without finding direction — 20min is insufficient), while time_expired exits at +5%+ (token was trending, just ran out of time) can re-enter sooner. Diagnosis: accumulating Phase 20 data to confirm the ±3% cohort WR before deploying the split-cooldown fix.
...
*Last stats update: 2026-04-03 06:55 UTC — **128 all-time trades, 46.9% WR** | Phase 1: +69.9% (57.1% WR, E=+3.33%) | Phase 3: 56.7% WR, -0.5% avg | Phase 5: 44.4% WR, -0.40% avg | **Phase 17:** Momentum 3.0x→2.0x — 9 trades, **55.6% WR, +1.7% avg, Sharpe 0.215, E=+1.69%** — broke negative-expectancy streak. | **Phase 18:** Liq cap $5M→$15M (liq cap calibrated to match 10% TP reality). | **Phase 19+20:** TP + time_expired re-entry blacklists — prevent DRV/TIBBIR double-dip patterns (-8% after +14.5% TP). **Phase 21 diagnosis in progress:** time_expired exits within ±3% of entry warrant 60min cooldown vs. 20min (sideways-drifted 6h token needs more reset time). Accumulating Phase 20 data before shipping. Recent 24h: **66.7% WR, +2.0% avg, E=+2.04%** — 21 evidence-based strategy iterations in 13 days. | Pitch deck: PITCH-DECK.md*
